    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::CreateEpic => {
                ...
                let create_epic_prompt = self.prompts.create_epic.deref();
                let epic = create_epic_prompt();
                self.db.create_epic(epic).with_context(|| anyhow!("could not create epic"))?;
            }
            Action::UpdateEpicStatus { epic_id } => {
                // prompt the user to update status and persist it in the database
                //let db_state = self.db.read_db()?;

                let update_epic_prompt = self.prompts.update_status.deref();
                let new_status = update_epic_prompt().with_context(|| anyhow!("no valid status given"))?;
                self.db.update_epic_status(epic_id, new_status).with_context(|| anyhow!("could not update epic status"))?;
            }
            Action::DeleteEpic { epic_id } => {
                // prompt the user to delete the epic and persist it in the database
                let delete_epic_prompt = self.prompts.delete_epic.deref();
                let delete = delete_epic_prompt();
                if delete {
            ...
            Action::CreateStory { epic_id } => {
                ...
                let create_story_prompt = self.prompts.create_story.deref();
            ...
            Action::UpdateStoryStatus { story_id } => {
                // prompt the user to update status and persist it in the database
                let update_story_status_prompt = self.prompts.update_status.deref();
                let status = update_story_status_prompt().with_context(|| anyhow!("could not parse status"))?;
                self.db.update_story_status(story_id, status).with_context(|| anyhow!("could not update story status"))?;
            }
            Action::DeleteStory { epic_id, story_id } => {
                // prompt the user to delete the story and persist it in the database
                let delete_story_prompt = self.prompts.delete_story.deref();
                let delete = delete_story_prompt();
                if delete {
