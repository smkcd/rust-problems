...
impl JiraDatabase {
    pub fn new(file_path: String) -> Self {
        Self {
            database: Box::new(JSONFileDatabase{file_path: file_path}),
        }
    }
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        // let epic = db.epics.get_mut(&epic_id);
        // if let Some(epic) = epic {
        //     epic.stories.push(story_id);
        // } else {
        //     return Err(Error::msg("epic id does not exist"));
        // }
        db.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("could not find epic in database!"))?.stories.push(story_id);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        let mut db = self.read_db()?;
        //let epic = db.epics.remove(&epic_id);
        // if let Some(epic) = epic {
        //     for story_id in epic.stories {
        //         db.stories.remove(&story_id).unwrap();
        //     }
        //     self.database.write_db(&db)?;
        //     return Ok(());
        // }
        // Err(Error::msg("epic id does not exist"))
    ...
    pub fn delete_story(&self,epic_id: u32, story_id: u32) -> Result<()> {
        ...
        //     } else {
        //         return Err(Error::msg("story does not exist in epic"));
        //     }
        //     // alternatively:
        //     // epic.stories.retain(|value| value != &story_id);
        // } else {
        //     return Err(Error::msg("epic id does not exist"));
        // }
        let epic = db.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("could not find epic in database!"))?;
        let story_index = epic
            .stories
            //.retain(|value| value != &story_id); does not return error if not found
            .iter()
            .position(|value| value == &story_id)
            .ok_or_else(|| anyhow!("could not find story within this epic in database!"))?;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut db = self.read_db()?;
        // if let Some(story) = db.stories.get_mut(&story_id) {
        //     story.status = status;
        //     self.database.write_db(&db)?;
        //     return Ok(());
        // } else {
        //     return Err(Error::msg("story id does not exist"));
        // }
        db.stories.get_mut(&story_id)
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        let db_data = fs::read_to_string(&self.file_path)?;
        let db_state:DBState = serde_json::from_str(&db_data)?;
        Ok(db_state)
    }

    fn write_db(&self, db_state: &DBState) -> Result<()> {
        // serialize db_state to json and store it in self.file_path
        let db_data = serde_json::to_string(db_state)?;
        let mut db_file = File::create(&self.file_path)?;
        db_file.write_all(db_data.as_bytes())?;
